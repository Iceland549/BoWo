...
namespace ContentMicroservice.Application.UseCases.Content
{
    ...
    public class ImportTrickFromYoutubeUseCase
    {
        ...
        public ImportTrickFromYoutubeUseCase(
            IContentRepository repo,
            IMapper mapper,
            ILogger<ImportTrickFromYoutubeUseCase> logger)
        {
            _repo = repo;
            _mapper = mapper;
            _logger = logger;
        ...
        public async Task<TrickDto> ExecuteAsync(string youtubeUrl, string authorId, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                if (created == null)
                {
                    _logger.LogWarning("YouTube import failed, creating local trick entry...");
                    created = await _repo.CreateTrickAsync(trick, ct);
                }

                _logger.LogInformation("Trick imported successfully: {TrickName}", created.Name);
